package stellarnear.aquene_dealer.Divers.SettingsFragments.DisplayStatsScreenFragment;
...
public class DSSFAtk {
    ...
    public DSSFAtk(View mainView, Context mC) {
        ...
        nAtkTxt.setText(aquene.getStats().getStatsList().getNAtksTot()+ " attaques");
    ...
    private BarDataSet computeBarDataSet(String mode) {
        ...
        int nAtkMax=0;
        for (Stat stat : aquene.getStats().getStatsList().asList()){
    ...
    private PieDataSet computePieDataSet() {
        ...
        if(nthAtkSelectedForPieChart==0){
            hitVal= aquene.getStats().getStatsList().getNAtksHit();
            missVal= aquene.getStats().getStatsList().getNAtksMiss();
            percent = 100f*(hitVal/(hitVal+missVal));
        } else {
            hitVal= aquene.getStats().getStatsList().getNAtksHitNthAtk(nthAtkSelectedForPieChart);
            missVal= aquene.getStats().getStatsList().getNAtksMissNthAtk(nthAtkSelectedForPieChart);
            percent = 100f*(hitVal/(hitVal+missVal));
        }
        if(percent>0f) {
    ...
    private PieDataSet computePieDataSetCrit() {
        ...
        if(nthAtkSelectedForPieChart==0){
            nHit= aquene.getStats().getStatsList().getNAtksHit();
            nCrit= aquene.getStats().getStatsList().getNCrit();
            nCritNat= aquene.getStats().getStatsList().getNCritNat();
        } else {
            nHit= aquene.getStats().getStatsList().getNAtksHitNthAtk(nthAtkSelectedForPieChart);
            nCrit= aquene.getStats().getStatsList().getNCritNth(nthAtkSelectedForPieChart);
            nCritNat= aquene.getStats().getStatsList().getNCritNatNth(nthAtkSelectedForPieChart);
        }
