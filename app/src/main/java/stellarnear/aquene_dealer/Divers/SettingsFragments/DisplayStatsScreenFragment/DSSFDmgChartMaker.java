...
import stellarnear.aquene_dealer.Divers.Elems.ElemsManager;
...
public class DSSFDmgChartMaker {
    ...
    public void buildChart() {
        ...
        formatAxisChart();
        if(aquene.getStats().getStatsList().size()>=1)addLimitsChart();
    ...
    private void calculateMinMaxRound() {
        ...
        if(elemsSelected.size()==2) {
            ...
            for (String elem:elemsSelected){
                int minElem= aquene.getStats().getStatsList().getMinDmgElem(elem);
                int maxElem= aquene.getStats().getStatsList().getMaxDmgElem(elem);
    ...
    private void addLimitLine(String elem) {
        ...
        if(elem.equalsIgnoreCase("all")){
            sumDmg = aquene.getStats().getStatsList().getLastStat().getSumDmg();
        } else {
            sumDmg = aquene.getStats().getStatsList().getLastStat().getElemSumDmg().get(elem);
        }
